"""
Upstage API 호출 함수 모음
- Tab 1(AI 마음 케어)과 Tab 3(AI 공백기 변환기)에서 사용해요.
- API 키는 반드시 st.secrets["UPSTAGE_API_KEY"]로만 불러오고, 코드에 직접 적지 않아요!
"""
...
원칙:
1. 사용자가 입력하지 않은 경험을 지어내거나 과장하지 않아요.
2. 실제 활동에서 드러나는 강점(꾸준함, 자기주도적 학습, 정보 습득력, 관찰력 등)만 진솔하게 짚어내세요.
3. '과장'이 아니라 '재해석'에 집중하세요.
4. 사용자가 관심 있다고 밝힌 분야가 있다면, 그 분야와 자연스럽게 연결지어 주세요.
5. 아래 형식을 정확히 지켜서 답변하세요.
...
def get_empathy_response(user_text: str) -> str:
    ...
    if client is None:
        return (
            "⚠️ 아직 Upstage API 키가 연결되지 않았어요. "
            ".streamlit/secrets.toml 파일에 UPSTAGE_API_KEY를 추가하면 AI 답변을 받을 수 있어요."
        )
...
def convert_gap_activity(user_text: str, interest_field: str = "") -> str:
    ...
    if client is None:
        return (
            "⚠️ 아직 Upstage API 키가 연결되지 않았어요. "
            ".streamlit/secrets.toml 파일에 UPSTAGE_API_KEY를 추가하면 AI 변환 결과를 받을 수 있어요."
        )
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": GAP_YEAR_SYSTEM_PROMPT,
                },
                {
                    "role": "user",
